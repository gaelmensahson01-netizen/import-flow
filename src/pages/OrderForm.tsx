...
export default function OrderForm() {
  ...
  const SectionHeader = ({ label }: { label: string }) => (
    <div className="bg-teal/10 border-l-4 border-teal px-4 py-2 rounded-r-lg mb-4">
  ...
  return (
    <div className="p-4 md:p-6 max-w-2xl mx-auto animate-fade-in">
      ...
      <h2 className="text-xl font-heading font-bold uppercase tracking-wider mb-6">{isEdit ? t('editOrder') : t('newOrder')}</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <section>
          ...
          <div className="grid gap-4">
            <div>
              <label className="block text-xs font-heading uppercase text-muted-foreground mb-1">{t('client')}</label>
              <input value={form.client} onChange={e => set('client', e.target.value)} required
                className="w-full h-10 px-3 rounded-lg bg-card border border-border text-card-foreground text-sm focus:border-teal focus:outline-none" />
            </div>
            <div>
              <label className="block text-xs font-heading uppercase text-muted-foreground mb-1">{t('transport')}</label>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-heading uppercase text-muted-foreground mb-1">{t('realPrice')}</label>
              <input type="number" value={form.realPrice || ''} onChange={e => set('realPrice', Number(e.target.value))}
                className="w-full h-10 px-3 rounded-lg bg-card border border-border text-card-foreground text-sm focus:border-teal focus:outline-none" />
            </div>
            <div>
              <label className="block text-xs font-heading uppercase text-muted-foreground mb-1">{t('clientPrice')}</label>
              <input type="number" value={form.clientPrice || ''} onChange={e => set('clientPrice', Number(e.target.value))}
                className="w-full h-10 px-3 rounded-lg bg-card border border-border text-card-foreground text-sm focus:border-teal focus:outline-none" />
            </div>
          </div>
          <div className="mt-3 p-3 rounded-lg bg-card border border-border">
            <span className="text-xs font-heading uppercase text-muted-foreground">{t('profit')}: </span>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 gap-4">
            {(['dateOrder','dateArrival','datePickup','dateDelivery'] as const).map(key => (
              <div key={key}>
                <label className="block text-xs font-heading uppercase text-muted-foreground mb-1">{t(key)}</label>
        ...
        <section>
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-heading uppercase text-muted-foreground mb-1">{t('rating')}</label>
              <StarRating value={form.rating} onChange={v => set('rating', v)} />
            </div>
            <div>
              <label className="block text-xs font-heading uppercase text-muted-foreground mb-1">{t('review')}</label>
              <textarea value={form.review} onChange={e => set('review', e.target.value)} rows={3}
                className="w-full px-3 py-2 rounded-lg bg-card border border-border text-card-foreground text-sm focus:border-teal focus:outline-none resize-none" />
            </div>
            <div>
              <label className="block text-xs font-heading uppercase text-muted-foreground mb-1">{t('suggestions')}</label>
        ...
        <div className="flex gap-3 pt-4">
          <button type="submit" className="flex-1 h-12 rounded-lg bg-teal text-primary-foreground font-heading font-bold uppercase tracking-wider hover:bg-teal-light transition-colors">
            {t('save')}
          </button>
          <button type="button" onClick={() => navigate(-1)} className="px-6 h-12 rounded-lg border-2 border-danger text-danger font-heading font-bold uppercase tracking-wider hover:bg-danger hover:text-primary-foreground transition-colors">
            {t('cancel')}
          </button>
        </div>
      </form>
